"""
Spark Streaming Job: Consume ZTM Trolleys from Kafka and Write to HBase

Reads from: ztm-trolleys-raw Kafka topic
...
print("🚀 Starting Spark Streaming: ZTM Trolleys → HBase")

# Read from Kafka
kafka_df = (
    spark.readStream.format("kafka")
    .option("kafka.bootstrap.servers", "kafka:29092")
    .option("subscribe", "ztm-trolleys-raw")
    .option("startingOffsets", "earliest")
    .option("failOnDataLoss", "false")
    .load()
)

print("✅ Connected to Kafka topic: ztm-trolleys-raw")
...
final_df = exploded_df.select(
    ...
    concat_ws(
        ...
        col("trolley.VehicleNumber"),
    ).alias("row_key"),
    # Column family: info
    col("trolley.Lines").alias("Lines"),
    col("trolley.VehicleNumber").alias("VehicleNumber"),
    col("trolley.Brigade").alias("Brigade"),
    col("trolley.Time").alias("Time"),
    # Column family: location
    col("trolley.Lat").cast("string").alias("Lat"),
    col("trolley.Lon").cast("string").alias("Lon"),
    # Column family: metadata (using defaults - no headers from NiFi)
    lit("trolley").alias("vehicle_type"),
...
query = (
    ...
    .outputMode("append")
    .option("checkpointLocation", "/tmp/spark-checkpoint-trolleys")
    .trigger(processingTime="30 seconds")
    .start()
)

print("🎯 Streaming query started!")
print("   Consuming from: ztm-trolleys-raw")
print("   Writing to: HBase transport_events")
print("   Checkpoint: /tmp/spark-checkpoint-trolleys")
